...
// Unblock fd's
inline int unblock (int fd){ int f;
...
inline UHashT UFNV32a(char *str){
...
inline UPair* new_UPair(char* key, void* value){
  UPair* i = malloc(sizeof(UPair)); i->key = key; i->value = value; return i; }

inline void free_UPair(UPair* p){
  if ( p ){
    if (p->key) free(p->key);
    if (p->value) free(p->value);
  }}

inline void UMap_insert(URecord* prev, URecord* item, URecord* next){
  next->prev = prev->next = item;
  item->prev = prev; item->next = next; }

inline void UMap_join(URecord* prev, URecord* next){
...
inline void* UMap_getI(UMap* map, UHashT hash){
  if ( !map ) _fatal("UMap_getI: empty map");
  URecord* rec = UMap_lookupI(map,hash);
  return rec ? rec->value : NULL; }

inline URecord* UMap_setI(UMap* map, UHashT hash, void* value){ int i,l; URecord *rec,*r;
...
inline URecord* UMap_lookup(UMap* map, char* key, UHashT* gethash){ URecord* rec; UHashT hash = NULL;
...
/* adapted from the busybox sources
    ...
    [last_char_is] Copyright 2001 Larry Doolittle, <ldoolitt@recycle.lbl.gov>
...
static char* concat_path_file(const char *path, const char *filename){ char *lc;
  if (!path) path = "";
  lc = last_char_is(path, '/');
  while (*filename == '/') filename++;
  asprintf(&lc, "%s%s%s", path, (lc==NULL ? "/" : ""), filename);
  return lc; }

static void delete_contents(const char *directory, dev_t rootdev){
  DIR *dir; struct dirent *d;  struct stat st;
  if (lstat(directory, &st) || st.st_dev != rootdev) return; // Don't descend into other filesystems
  if (S_ISDIR(st.st_mode)) {                       // Recursively delete the contents of directories
    dir = opendir(directory);
    if (dir) {
      while ((d = readdir(dir))) {
        char *newdir = d->d_name;
        if (DOT_OR_DOTDOT(newdir)) continue;                                        // Skip . and ..
        newdir = concat_path_file(directory, newdir);
        delete_contents(newdir, rootdev);                              // Recurse to delete contents
        free(newdir); }
      closedir(dir);                                        // Directory should now be empty, zap it
      rmdir(directory); }
  } else { unlink(directory); }}                                    // It wasn't a directory, zap it
...
void switchroot(ioEvent* e) {
  ...
  if (stat("/init", &st) != 0 || !S_ISREG(st.st_mode)) {
    ULOG("/init is not a regular file"); return; }
  statfs("/", &stfs); // this never fails
  if ((unsigned)stfs.f_type != RAMFS_MAGIC
   && (unsigned)stfs.f_type != TMPFS_MAGIC ){
    ULOG("root filesystem is not ramfs/tmpfs"); return; }           // Zap everything out of rootdev
  delete_contents("/", rootdev);                       // Overmount / with newdir and chroot into it
